use flat;
use params::{EarthShape, Params, RayDir};
use spherical;
use PI;
...
fn ray_from_shape_h_ang(shape: &EarthShape, h0: f64, angle: f64) -> Box<Path> {
    match *shape {
        EarthShape::Spherical { radius } => Box::new(spherical::Ray::from_h_ang(radius, h0, angle)),
        EarthShape::Flat => Box::new(flat::Ray::from_h_ang(h0, angle)),
    }
}
...
fn path_from_h_ang(shape: &EarthShape, straight: bool, h0: f64, angle: f64) -> Box<Path> {
    if straight {
        line_from_shape_h_ang(shape, h0, angle)
    } else {
        ray_from_shape_h_ang(shape, h0, angle)
    }
}

fn find_angle_to_target(shape: &EarthShape, h0: f64, tgt_h: f64, tgt_dist: f64) -> f64 {
    let (mut min_ang, mut max_ang) = (-1.5, 1.5);

    while max_ang - min_ang > 0.000001 {
        let cur_ang = 0.5 * (min_ang + max_ang);
        let ray = ray_from_shape_h_ang(shape, h0, cur_ang);
...
fn path_from_h_tgt(
    ...
    if straight {
        ...
        let ang = find_angle_to_target(shape, h0, tgt_h, tgt_dist);
        ray_from_shape_h_ang(shape, h0, ang)
    }
}
...
pub fn create_path(params: &Params) -> Box<Path> {
    match params.ray.dir {
        ...
        RayDir::Target { h, dist } => {
            ...
            path_from_h_tgt(&params.shape, params.straight, params.ray.start_h, h, dist)
        }
        RayDir::Horizon => {
            if params.verbose {
